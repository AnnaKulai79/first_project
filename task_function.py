...
# tag = str_text.split()[0]
# text = " ".join(str_text.split()[1:])
# print(create_tags(tag, text)) 
...
# key invalid
def valid_key(key):
    return all(ch.isalpha() for ch in key)
def vigenere_cipher(key,text):
    ...
    for i in range(len(text)):
        zsuv = dictionary.find(key[j])
        num_shufr = dictionary.find(text[i].lower())
        nov_ind = zsuv + num_shufr - 25 * ((zsuv + num_shufr)//26) 
        if text[i].isupper():
            res += dictionary[nov_ind].upper()
            j += 1
        elif text[i] in exep:
            res += text[i]    
        else:
...
def cezar_cipher(key,text):
    ...
    for i in range(len(text)):
        num_shufr = dictionary.find(text[i].lower())
        nov_ind = int(key) + num_shufr - 26 * ((int(key) + num_shufr)//26) 
        if text[i].isupper():
            res += dictionary[nov_ind].upper()
        elif text[i] in exep:
            res += text[i]    
...
# Напишіть функцію, яка з послідовності додатних цілих чисел складає мінімальне і максимальне числа як у вихідних даних.
# Вхідні дані:
# 2 0 509 23
# 45 0 14 8 1
# Вихідні дані:
